...
class StagedIndexReader(emb: EmitMethodBuilder[_], spec: AbstractIndexSpec) {
  ...
  def queryInterval(cb: EmitCodeBuilder,
    ...
    info("INDEXDEBUG: starting interval query for interval=", cb.strValue(interval), ", idx nkeys=", nKeys.toS)
    ...
    info("INDEXDEBUG: start query result=", startIdx.toS)
    info("INDEXDEBUG: end query result=", endIdx.toS)
  ...
  private def queryBound(cb: EmitCodeBuilder,
    ...
    info("INDEXDEBUG: level=", levelSettable.toS, ", offset=", offsetSettable.toS, ", boundAndSign=", cb.strValue(boundAndSignTuple))
    cb.ifx(levelSettable ceq 0, {
      ...
      info("INDEXDEBUG: read internal node at ", offset.toS, ", with ", children.loadLength().toS, " children")
      cb.ifx(children.loadLength() ceq 0, {
        ...
        val idx = new BinarySearch(cb.emb,
          ...
          .search(cb, children, EmitCode.present(cb.emb, boundAndSignTuple))
        info("INDEXDEBUG: internal node idx = ", idx.toS)
