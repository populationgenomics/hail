...
class StagedIndexReader(emb: EmitMethodBuilder[_], spec: AbstractIndexSpec) {
  ...
  def queryInterval(cb: EmitCodeBuilder,
    ...
    info(s"INDEXDEBUG: starting interval query for interval=${cb.strValue(interval).toString}, idx nkeys=${nKeys.toS.toString}")

    info(s"INDEXDEBUG: leaf at 0=${cb.strValue(queryIndex(cb, region, 0L)).toString}")
    info(s"INDEXDEBUG: leaf at nKeys-1=${cb.strValue(queryIndex(cb, region, cb.memoize(nKeys-1))).toString}")
    ...
    info(s"INDEXDEBUG: start query result=${startIdx.toS.toString}")
    info(s"INDEXDEBUG: end query result=${endIdx.toS.toString}")
  ...
  private def queryBound(cb: EmitCodeBuilder,
    ...
    info(s"INDEXDEBUG: level=${levelSettable.toS.toString}, offset=${offsetSettable.toS.toString}, boundAndSign=${cb.strValue(boundAndSignTuple).toString}")
    cb.ifx(levelSettable ceq 0, {
      ...
      info(s"INDEXDEBUG: read internal node at ${offset.toS.toString}, with ${children.loadLength().toS.toString} children")
      cb.ifx(children.loadLength() ceq 0, {
        ...
        val idx = new BinarySearch(cb.emb,
          ...
          .search(cb, children, EmitCode.present(cb.emb, boundAndSignTuple))
        info(s"INDEXDEBUG: internal node idx=${idx.toS.toString}")
