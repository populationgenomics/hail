...
class StagedIndexReader(emb: EmitMethodBuilder[_], spec: AbstractIndexSpec) {
  ...
  def queryInterval(cb: EmitCodeBuilder,
    ...
    info(s"INDEXDEBUG: starting interval query for interval=${cb.strValue(interval)}, idx nkeys=${nKeys.toS}")

    info(s"INDEXDEBUG: leaf at 0=${cb.strValue(queryIndex(cb, region, 0L))}")
    info(s"INDEXDEBUG: leaf at nKeys-1=${cb.strValue(queryIndex(cb, region, cb.memoize(nKeys-1)))}")
    ...
    info(s"INDEXDEBUG: start query result=${startIdx.toS}")
    info(s"INDEXDEBUG: end query result=${endIdx.toS}")
  ...
  private def queryBound(cb: EmitCodeBuilder,
    ...
    info(s"INDEXDEBUG: level=${levelSettable.toS}, offset=${offsetSettable.toS}, boundAndSign=${cb.strValue(boundAndSignTuple)}")
    cb.ifx(levelSettable ceq 0, {
      ...
      info(s"INDEXDEBUG: read internal node at ${offset.toS}, with ${children.loadLength().toS} children")
      cb.ifx(children.loadLength() ceq 0, {
        ...
        val idx = new BinarySearch(cb.emb,
          ...
          .search(cb, children, EmitCode.present(cb.emb, boundAndSignTuple))
        info(s"INDEXDEBUG: internal node idx=${idx.toS}")
