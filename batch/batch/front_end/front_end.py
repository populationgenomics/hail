...
REQUEST_TIME = pc.Summary(
    'batch_request_latency_seconds',
    'Batch request latency in seconds',
    ['endpoint', 'verb'],
)
REQUEST_TIME_GET_JOBS = REQUEST_TIME.labels(
    endpoint='/api/v1alpha/batches/batch_id/jobs', verb="GET"
)
...
REQUEST_TIME_GET_ATTEMPTS = REQUEST_TIME.labels(
    endpoint='/api/v1alpha/batches/batch_id/jobs/job_id/attempts', verb="GET"
)
REQUEST_TIME_GET_BATCHES = REQUEST_TIME.labels(
    endpoint='/api/v1alpha/batches', verb="GET"
)
...
REQUEST_TIME_POST_GET_BATCH = REQUEST_TIME.labels(
    endpoint='/api/v1alpha/batches/batch_id', verb='GET'
)
REQUEST_TIME_PATCH_CANCEL_BATCH = REQUEST_TIME.labels(
    endpoint='/api/v1alpha/batches/batch_id/cancel', verb="PATCH"
)
REQUEST_TIME_PATCH_CLOSE_BATCH = REQUEST_TIME.labels(
    endpoint='/api/v1alpha/batches/batch_id/close', verb="PATCH"
)
REQUEST_TIME_DELETE_BATCH = REQUEST_TIME.labels(
    endpoint='/api/v1alpha/batches/batch_id', verb="DELETE"
)
REQUEST_TIME_GET_BATCH_UI = REQUEST_TIME.labels(
    endpoint='/batches/batch_id', verb='GET'
)
REQUEST_TIME_POST_CANCEL_BATCH_UI = REQUEST_TIME.labels(
    endpoint='/batches/batch_id/cancel', verb='POST'
)
REQUEST_TIME_POST_DELETE_BATCH_UI = REQUEST_TIME.labels(
    endpoint='/batches/batch_id/delete', verb='POST'
)
REQUEST_TIME_GET_BATCHES_UI = REQUEST_TIME.labels(endpoint='/batches', verb='GET')
REQUEST_TIME_GET_JOB_UI = REQUEST_TIME.labels(
    endpoint='/batches/batch_id/jobs/job_id', verb="GET"
)
REQUEST_TIME_GET_BILLING_PROJECTS = REQUEST_TIME.labels(
    endpoint='/api/v1alpha/billing_projects', verb="GET"
)
REQUEST_TIME_GET_BILLING_PROJECT = REQUEST_TIME.labels(
    endpoint='/api/v1alpha/billing_projects/billing_project', verb="GET"
)
REQUEST_TIME_GET_BILLING_LIMITS_UI = REQUEST_TIME.labels(
    endpoint='/billing_limits', verb="GET"
)
REQUEST_TIME_POST_BILLING_LIMITS_EDIT_UI = REQUEST_TIME.labels(
    endpoint='/billing_projects/billing_project/edit', verb="POST"
)
REQUEST_TIME_POST_BILLING_LIMITS_EDIT = REQUEST_TIME.labels(
    endpoint='/api/v1alpha/billing_projects/billing_project/edit', verb="POST"
)
REQUEST_TIME_GET_BILLING_UI = REQUEST_TIME.labels(endpoint='/billing', verb="GET")
REQUEST_TIME_GET_BILLING_PROJECTS_UI = REQUEST_TIME.labels(
    endpoint='/billing_projects', verb="GET"
)
REQUEST_TIME_POST_BILLING_PROJECT_REMOVE_USER_UI = REQUEST_TIME.labels(
    endpoint='/billing_projects/billing_project/users/user/remove', verb="POST"
)
REQUEST_TIME_POST_BILLING_PROJECT_REMOVE_USER_API = REQUEST_TIME.labels(
    endpoint='/api/v1alpha/billing_projects/billing_project/users/{user}/remove',
    verb="POST",
)
REQUEST_TIME_POST_BILLING_PROJECT_ADD_USER_UI = REQUEST_TIME.labels(
    endpoint='/billing_projects/billing_project/users/add', verb="POST"
)
REQUEST_TIME_POST_BILLING_PROJECT_ADD_USER_API = REQUEST_TIME.labels(
    endpoint='/api/v1alpha/billing_projects/{billing_project}/users/{user}/add',
    verb="POST",
)
REQUEST_TIME_POST_CREATE_BILLING_PROJECT_UI = REQUEST_TIME.labels(
    endpoint='/billing_projects/create', verb="POST"
)
REQUEST_TIME_POST_CREATE_BILLING_PROJECT_API = REQUEST_TIME.labels(
    endpoint='/api/v1alpha/billing_projects/{billing_project}/create', verb="POST"
)
REQUEST_TIME_POST_CLOSE_BILLING_PROJECT_UI = REQUEST_TIME.labels(
    endpoint='/billing_projects/{billing_project}/close', verb="POST"
)
REQUEST_TIME_POST_CLOSE_BILLING_PROJECT_API = REQUEST_TIME.labels(
    endpoint='/api/v1alpha/billing_projects/{billing_project}/close', verb="POST"
)
REQUEST_TIME_POST_REOPEN_BILLING_PROJECT_UI = REQUEST_TIME.labels(
    endpoint='/billing_projects/{billing_project}/reopen', verb="POST"
)
REQUEST_TIME_POST_REOPEN_BILLING_PROJECT_API = REQUEST_TIME.labels(
    endpoint='/api/v1alpha/billing_projects/{billing_project}/reopen', verb="POST"
)
REQUEST_TIME_POST_DELETE_BILLING_PROJECT_API = REQUEST_TIME.labels(
    endpoint='/api/v1alpha/billing_projects/{billing_project}/reopen', verb="POST"
)
...
BATCH_JOB_DEFAULT_WORKER_TYPE = os.environ.get(
    'HAIL_BATCH_JOB_DEFAULT_WORKER_TYPE', 'standard'
)
...
'''
    ...
    jobs = [
        job_record_to_dict(record, record['name'])
...
async def _get_job_log_from_record(app, batch_id, job_id, record):
    ...
    if state in ('Error', 'Failed', 'Success'):
        ...
        async def _read_log_from_gcs(task):
            try:
                data = await log_store.read_log_file(
...
'''
    ...
    batches = [
        ...
        async for batch in db.select_and_fetchall(sql, sql_args)
    ]
...
def check_service_account_permissions(user, sa):
    ...
    raise web.HTTPBadRequest(
        reason=f'unauthorized service account {(sa["namespace"], sa["name"])} for user {user}'
    )
...
''',
        ...
        async with timer.step('build db args'):
            ...
            for spec in job_specs:
                ...
                if req_cores_mcpu == 0:
                    raise web.HTTPBadRequest(
                        reason=f'bad resource request for job {id}: ' f'cpu cannot be 0'
                    )
                ...
                if worker_type:
                    ...
                    result = inst_coll_configs.select_job_private(
                        machine_type=machine_type, storage_bytes=req_storage_bytes
                    )
                    if result:
                ...
                if len(secrets) != 0 and user != 'ci':
                    secrets = [
                        (secret["namespace"], secret["name"]) for secret in secrets
                    ]
                    raise web.HTTPBadRequest(
                        reason=f'unauthorized secret {secrets} for user {user}'
                    )

                for secret in secrets:
                    if user != 'ci':
                        raise web.HTTPBadRequest(
                            reason=f'unauthorized secret {(secret["namespace"], secret["name"])}'
                        )
                ...
                if all(
                    envvar['name'] != 'GOOGLE_APPLICATION_CREDENTIALS'
                    for envvar in spec['env']
                ):
                    spec['env'].append(
                        {
...
''',
                ...
                for inst_coll, resources in inst_coll_resources.items():
                    ...
                    ready_cancellable_cores_mcpu = resources[
...
async def create_batch(request, userdata):
    ...
    @transaction(db)
    async def insert(tx):
        billing_projects = await query_billing_projects(
            tx, user=user, billing_project=billing_project
        )
        ...
        if billing_projects[0]['status'] in {'closed', 'deleted'}:
            raise web.HTTPForbidden(
                reason=f'Billing project {billing_project} is closed or deleted.'
            )
...
''',
    ...
    async with client_session() as session:
        await request_retry_transient_errors(
            ...
            'PATCH',
            deploy_config.url(
                'batch-driver', f'/api/v1alpha/batches/{user}/{batch_id}/close'
            ),
...
async def ui_cancel_batch(
    request, userdata, batch_id
...
async def ui_batches(request, userdata):
    ...
    return await render_template(
        'batch', request, userdata, 'batches.html', page_context
    )
...
async def ui_get_job(request, userdata, batch_id):
    ...
    job_status, attempts, job_log = await asyncio.gather(
    ...
    if 'process' in job_specification:
        ...
        job_specification['image'] = (
            process_specification['image'] if process_type == 'docker' else '[jvm]'
        )
        job_specification['command'] = process_specification['command']
    job_specification = dictfix.dictfix(
        job_specification,
        dictfix.NoneOr(
            {'image': str, 'command': list, 'resources': dict(), 'env': list}
        ),
    )
...
async def ui_get_billing_limits(request, userdata):
    ...
    return await render_template(
        'batch', request, userdata, 'billing_limits.html', page_context
    )
...
async def post_edit_billing_limits(
    request, userdata
):  # pylint: disable=unused-argument
...
async def post_edit_billing_limits_ui(
    request, userdata
):  # pylint: disable=unused-argument
    db: Database = request.app['db']
    billing_project = request.match_info['billing_project']
    ...
    limit = post['limit']
    session = await aiohttp_session.get_session(request)
    errored = await _handle_ui_error(
        session, _edit_billing_limit, db, billing_project, limit
    )
    if not errored:
        set_message(
            session,
            f'Modified limit {limit} for billing project {billing_project}.',
            'info',
        )
...
async def _query_billing(request):
    ...
    try:
        ...
        return await parse_error(
            f"Invalid value for start '{start_query}'; must be in the format of MM/DD/YYYY."
        )
    ...
    try:
        ...
        return await parse_error(
            f"Invalid value for end '{end_query}'; must be in the format of MM/DD/YYYY."
        )
...
async def ui_get_billing(request, userdata):
    ...
    for record in billing:
        ...
        billing_by_project[billing_project] = (
            billing_by_project.get(billing_project, 0) + cost
        )
    ...
    billing_by_user = [
        {'user': user, 'cost': cost_str(cost)} for user, cost in billing_by_user.items()
    ]
    ...
    billing_by_project_user.sort(
        key=lambda record: (record['billing_project'], record['user'])
    )
    ...
    return await render_template(
        'batch', request, userdata, 'billing.html', page_context
    )
...
async def ui_get_billing_projects(request, userdata):
    ...
    return await render_template(
        'batch', request, userdata, 'billing_projects.html', page_context
    )
...
async def get_billing_project(request, userdata):
    ...
    billing_projects = await query_billing_projects(
        db, user=user, billing_project=billing_project
    )
...
''',
        ...
        if row['status'] in {'closed', 'deleted'}:
            raise BatchUserError(
                f'Billing project {billing_project} has been closed or deleted and cannot be modified.',
                'error',
            )
...
@check_csrf_token
@web_authenticated_developers_only(redirect=False)
async def post_billing_projects_remove_user(
    request, userdata
):  # pylint: disable=unused-argument
    db: Database = request.app['db']
    billing_project = request.match_info['billing_project']
    ...
    session = await aiohttp_session.get_session(request)
    errored = await _handle_ui_error(
        session, _remove_user_from_billing_project, db, billing_project, user
    )
    if not errored:
        set_message(
            session,
            f'Removed user {user} from billing project {billing_project}.',
            'info',
        )
...
@rest_authenticated_developers_or_auth_only
async def api_get_billing_projects_remove_user(
    request, userdata
):  # pylint: disable=unused-argument
    db: Database = request.app['db']
    billing_project = request.match_info['billing_project']
    ...
    await _handle_api_error(
        _remove_user_from_billing_project, db, billing_project, user
    )
...
@check_csrf_token
@web_authenticated_developers_only(redirect=False)
async def post_billing_projects_add_user(
    request, userdata
):  # pylint: disable=unused-argument
    db: Database = request.app['db']
    post = await request.post()
    ...
    errored = await _handle_ui_error(
        session, _add_user_to_billing_project, db, billing_project, user
    )
    if not errored:
        set_message(
            session, f'Added user {user} to billing project {billing_project}.', 'info'
        )
    return web.HTTPFound(deploy_config.external_url('batch', '/billing_projects'))
...
@rest_authenticated_developers_or_auth_only
async def api_billing_projects_add_user(
    request, userdata
):  # pylint: disable=unused-argument
    db: Database = request.app['db']
    user = request.match_info['user']
...
''',
        ...
        if row is not None:
            raise BatchOperationAlreadyCompletedError(
                f'Billing project {billing_project} already exists.', 'info'
            )
...
@check_csrf_token
@web_authenticated_developers_only(redirect=False)
async def post_create_billing_projects(
    request, userdata
):  # pylint: disable=unused-argument
    db: Database = request.app['db']
    post = await request.post()
    ...
    session = await aiohttp_session.get_session(request)
    errored = await _handle_ui_error(
        session, _close_billing_project, db, billing_project
    )
    if not errored:
...
@rest_authenticated_developers_or_auth_only
async def api_get_create_billing_projects(
    request, userdata
):  # pylint: disable=unused-argument
    db: Database = request.app['db']
    billing_project = request.match_info['billing_project']
...
WHERE name = %s LIMIT 1 FOR UPDATE;
    ''',
        ...
        if row['status'] == 'closed':
            raise BatchOperationAlreadyCompletedError(
                f'Billing project {billing_project} is already closed or deleted.',
                'info',
            )
        if row['batch_id'] is not None:
            raise BatchUserError(
                f'Billing project {billing_project} has open or running batches.',
                'error',
            )
...
@check_csrf_token
@web_authenticated_developers_only(redirect=False)
async def post_close_billing_projects(
    request, userdata
):  # pylint: disable=unused-argument
    db: Database = request.app['db']
    billing_project = request.match_info['billing_project']
...
@rest_authenticated_developers_or_auth_only
async def api_close_billing_projects(
    request, userdata
):  # pylint: disable=unused-argument
    db: Database = request.app['db']
    billing_project = request.match_info['billing_project']
...
async def _reopen_billing_project(db, billing_project):
    ...
    async def open_project(tx):
        ...
        if row['status'] == 'deleted':
            raise BatchUserError(
                f'Billing project {billing_project} has been deleted and cannot be reopened.',
                'error',
            )
        if row['status'] == 'open':
            raise BatchOperationAlreadyCompletedError(
                f'Billing project {billing_project} is already open.', 'info'
            )
...
@check_csrf_token
@web_authenticated_developers_only(redirect=False)
async def post_reopen_billing_projects(
    request, userdata
):  # pylint: disable=unused-argument
    db: Database = request.app['db']
    billing_project = request.match_info['billing_project']
    ...
    session = await aiohttp_session.get_session(request)
    errored = await _handle_ui_error(
        session, _reopen_billing_project, db, billing_project
    )
    if not errored:
...
@rest_authenticated_developers_or_auth_only
async def api_reopen_billing_projects(
    request, userdata
):  # pylint: disable=unused-argument
    db: Database = request.app['db']
    billing_project = request.match_info['billing_project']
...
async def _delete_billing_project(db, billing_project):
    ...
    async def delete_project(tx):
        ...
        if row['status'] == 'deleted':
            raise BatchOperationAlreadyCompletedError(
                f'Billing project {billing_project} is already deleted.', 'info'
            )
        if row['status'] == 'open':
            raise BatchUserError(
                f'Billing project {billing_project} is open and cannot be deleted.',
                'error',
            )
...
@rest_authenticated_developers_or_auth_only
async def api_delete_billing_projects(
    request, userdata
):  # pylint: disable=unused-argument
    db: Database = request.app['db']
    billing_project = request.match_info['billing_project']
...
'''
    ...
    app['driver_headers'] = {'Authorization': f'Bearer {row["internal_token"]}'}

    credentials = google.oauth2.service_account.Credentials.from_service_account_file(
        '/gsa-key/key.json'
    )
    app['log_store'] = LogStore(
        BATCH_BUCKET_NAME, instance_id, pool, credentials=credentials
    )
    ...
    app['task_manager'].ensure_future(
        retry_long_running(
            'delete_batch_loop',
            run_if_changed,
            delete_batch_state_changed,
            delete_batch_loop_body,
            app,
        )
    )
...
def run():
    ...
    web.run_app(
        deploy_config.prefix_application(
            app, 'batch', client_max_size=HTTP_CLIENT_MAX_SIZE
        ),
        host='0.0.0.0',
        port=5000,
